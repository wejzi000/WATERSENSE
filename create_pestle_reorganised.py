...
# -*- coding: utf-8 -*-
"""
ANALYSE PESTLE DÉTAILLÉE WATERSENSE 2026 (Réorganisée)
Ordre: P-E-S-T-L-E (Légal AVANT Écologie)
"""
...
def create_pestle_reorganised():
    """Analyse PESTLE réorganisée (L avant E)"""
    doc = Document()
    ...
    title_run = title.add_run("ANALYSE PESTLE DÉTAILLÉE\n(Réorganisée P-E-S-T-L-E)")
    ...
    desc_run = desc.add_run("Analyse macro-environnementale complète\n18 Facteurs Politiques, Économiques, Sociaux, Technologiques, Légaux, Écologiques")
    ...
    meta_run = meta.add_run("Février 2026\nTableau PESTLE Réorganisé\nL (Légal) avant E (Écologie)")
    ...
    # INTRODUCTION
    # ========================================================================
    add_colored_heading(doc, "ANALYSE PESTLE RÉORGANISÉE - WATERSENSE 2026", level=1)
    
    add_paragraph_formatted(doc,
        "Tableau PESTLE avec ordre réorganisé : P (Politique) → E (Économique) → S (Social) → T (Technologie) → L (Légal) → E (Écologie).",
        size=11)
    
    doc.add_paragraph()
    
    add_colored_heading(doc, "TABLEAU PESTLE COMPLET (18 FACTEURS - RÉORGANISÉ)", level=2)
    
    # Create PESTLE table with reorganised order
    pestle_data = [
        # Header
        ...
        ["P", "Subvention CAP 2023-2027\n(Bonus agritech irrigation)", 
         "Bonus +25% subvention base (2.5K-6K€/exploitation) si irrigation 'raisonnée'. Définition raisonnée = données temps réel + technologie certifiée.",
         "TRÈS FORT", "CERTAINE", "0", "+5",
         "Avantage majeur : WaterSense déverrouille bonus CAP = ROI accéléré"],
        
        ["P", "Gouvernance locale eau\n(Bassins hydrographiques)", 
         "Comités bassins rivière (Rhin, Loire, Rhône, Méditerranée) réduisent allocations. Variations régionales -15% Nord vs -30% Sud.",
         "FORT", "CERTAINE", "-2", "+3",
         "Focus commercial régions Sud prioritaires"],
        
        # ÉCONOMIQUE (4 facteurs)
        ["E", "Prix céréales effondrés\n(2022-2026)", 
         "Maïs 210€/t (vs 310€/t 2022, -32%). Blé 230€/t (vs 380€/t 2022, -39%). Marges agriculteur réduites drastiquement.",
         "TRÈS FORT", "CERTAINE", "-3", "+4",
         "Agriculteurs DOIVENT optimiser eau (+ revenu seul levier). Urgence maximale"],
        
        ["E", "Coûts production agriculture\n(+150% depuis 2020)", 
         "Électricité +180%, Engrais +220%, Diesel +85%. Coût prod/hectare 2,500€ → 3,750€. Seuil rentabilité augmente 30%.",
         "TRÈS FORT", "CERTAINE", "0", "+4",
         "Économie eau = économie DIRECTE. Chaque m³ économisé = profit net"],
        
        ["E", "Endettement agricole massif\n(54% surendettées)", 
         "54% exploitations surendettées (vs 35% en 2015). Taux intérêt 4.2% (vs 2.5% en 2020). Banques resserrent crédits.",
         "FORT", "ÉLEVÉE", "-2", "0",
         "Modèle SaaS (pas capex lourd) = attrayant pour cash-strapped exploitations"],
        
        ["E", "Investissement agritech\n(Croissance +27% CAGR)", 
         "Marché software irrigation France +27% CAGR. Budget investissement agritech 950M€ (vs 840M€ 2023). Tech = levier rentabilité.",
         "FORT", "CROISSANTE", "0", "+5",
         "WaterSense tappe marché croissance exponentiel. Timing opportun"],
        
        # SOCIAL (3 facteurs)
        ["S", "Acceptation technologie\n(Agriculteurs <55 ans)", 
         "Jeunes agriculteurs (<40 ans) adopte tech facilement. 35% agriculteurs <55 ans tech-friendly. Early adopters (12%) cherchent solutions sophistiquées.",
         "FORT", "CROISSANTE", "0", "+5",
         "Early adopters = first customers. Target jeunes agri + moyen"],
        
        ["S", "Résistance technologie\n(Agriculteurs >55 ans)", 
         "45% agriculteurs >55 ans résistant adoption tech. Peurs : hacking données, coût, complexité. Besoin formation 20-30h.",
         "MOYEN", "ÉLEVÉE", "-2", "0",
         "Support français local + formation gratuite = mitigation risque"],
        
        ["S", "Demande RSE/Durabilité\n(Agriculteurs modernes)", 
         "Demande croissante responsabilité environnementale. Clients B2B exigent certifications ESG. Consommateurs demandent transparence eau.",
         "FORT", "CROISSANTE", "0", "+5",
         "WaterSense positionnement RSE (réduction eau -30%, neutralité C 2025) = différenciation"],
        
        # TECHNOLOGIE (3 facteurs)
        ["T", "Maturité IoT & IA\n(Coûts baisse 25%/an)", 
         "Coûts capteurs baissent 25%/an. IA/ML modèles pré-trainés (TensorFlow). Cloud AWS coûts compétitifs scalables.",
         "TRÈS FORT", "CERTAINE", "0", "+5",
         "Technologie mûre + accessible. Timing idéal lancement WaterSense"],
        
        ["T", "Couverture réseau rural\n(4G/5G zones agricoles)", 
         "Couverture 4G zones irrigables 85% (vs 50% 2015). 5G 2026 couvre 40% zones agricoles. Fallback WiFi/LoRa offline.",
         "FORT", "CROISSANTE", "0", "+4",
         "Infrastructure réseau mature. Transmission données fiable"],
        
        ["T", "Cybersécurité & Data Privacy\n(Normes strictes)", 
         "RGPD + ISO 27001 OBLIGATOIRE. Données agricoles sensibles. Certification sécurité = entrée marché REQUIREMENT.",
        ...
        ["L", "RGPD & Sécurité données\n(Données agricoles sensibles)", 
         "RGPD strict s'applique données agricoles. Data stockage EU zone seulement (AWS eu-west-1). Amendes CNIL 4% CA.",
         "FORT", "CROISSANTE", "-3", "0",
         "Conformité RGPD = coût préalable. WaterSense prêt = advantage"],
        
        ["L", "Norme CE équipement\n(ISO 61 326-2, 2025+)", 
         "Norme CE ISO 61 326-2 obligatoire 2025+. Certification 3-6 mois coûte 15K€-25K€. Barrière entrée SIGNIFICANTE.",
         "MOYEN", "CERTAINE", "0", "+2",
         "WaterSense certification CE Q1 2026 = early mover advantage"],
        
        ["L", "Contrats agriculture & IP\n(Propriété données)", 
         "Propriété données agricoles = ENJEU légal 2026. Débat UE : données = propriété agriculteur vs startup. WaterSense contrats clairs.",
         "MOYEN", "ÉLEVÉE", "-1", "+2",
        ...
        ["E", "Pénuries eau estivales\n(Zones méditerranéennes)", 
         "Sécheresses intensifiées 2022-2026. Été 2025 = PIRE crise 60 ans. Allocations eau -40-60% zones méditerranéennes 2026.",
         "TRÈS FORT", "CERTAINE", "0", "+5",
         "Pénurie eau = BESOIN #1. WaterSense = réponse directe"],
        
        ["E", "Objectifs neutralité carbone\n(Fit55, UE 2030)", 
         "Fit for 55 oblige agriculture France -40% émissions 2030. Pompes diesel → électrique conversion 2028. Green Deal 2050 carbon neutral.",
         "TRÈS FORT", "CERTAINE", "0", "+4",
    ...
    for i, header_text in enumerate(pestle_data[0]):
    ...
    # Data rows
    for row_idx, row_data in enumerate(pestle_data[1:], 1):
    ...
    # SYNTHÈSE SCORING
    # ========================================================================
    add_colored_heading(doc, "SYNTHÈSE SCORING PESTLE", level=2)
    ...
    output_file = "ANALYSE_PESTLE_REORGANISEE_WATERSENSE_2026.docx"
    doc.save(output_file)
    return output_file

if __name__ == '__main__':
    print("╔════════════════════════════════════════════════════════════╗")
    print("║  GÉNÉRATION PESTLE RÉORGANISÉE (L avant E)                ║")
    print("║  Ordre : P-E-S-T-L-E                                      ║")
    print("╚════════════════════════════════════════════════════════════╝")
    print()
    
    output_file = create_pestle_reorganised()
    
    print(f"✅ PESTLE réorganisée créée : {output_file}")
    print()
    print("📊 ORDRE RÉORGANISÉ :")
    print("  ✓ P (Politique) - 3 facteurs")
    print("  ✓ E (Économique) - 4 facteurs")
    print("  ✓ S (Social) - 3 facteurs")
    print("  ✓ T (Technologie) - 3 facteurs")
    print("  ✓ L (Légal) - 3 facteurs ← AVANT")
    print("  ✓ E (Écologie) - 2 facteurs ← APRÈS")
    print()
    print("════════════════════════════════════════════════════════════")
    print("✅ PESTLE RÉORGANISÉE GÉNÉRÉE (L avant E)")
    print("════════════════════════════════════════════════════════════")
